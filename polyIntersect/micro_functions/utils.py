...
def buffer_ogr_polygons(ogr_geom, distance, dissolve=True, original_epsg=4326):
...
def project(ogr_geom, centroid, direction, original_epsg):
    ...
    if direction.lower() == 'to-custom':
        ...
        raise Exception('utils.project \'direction\' parameter invalid. \
...
def build_buffer(ogr_geom, distance, original_epsg, export_as='OGR',
                 return_to_original_sr=True):
...
def calculate_dissolved_area(ogr_geom, original_epsg=4326):
    ...
    if ogr_geom.GetGeometryType() == 6:  # Geometry type 6 is MultiPolygon
        ...
        for i in range(0, parts):
            ...
            area_part = project(geom, geom.Centroid(), 'to-custom',
                                original_epsg).GetArea()
            area_m2 += area_part

    else:  # Geometry type must be Polygon (type 3) here
        area_m2 = project(ogr_geom, ogr_geom.Centroid(), 'to-custom',
                          original_epsg).GetArea()
...
# get_min_max_xy assumes ogr_geom is passed in with decimal degrees
# as the linear unit, so that decimal degrees are returned as min/max x/y
def get_min_max_xy(ogr_geom):
    envelope = ogr_geom.GetEnvelope()
    return envelope


def get_intersect_geom_from_endpoint(ogr_geom, layer, fields,
                                     original_epsg=4326):
    ...
    if layer == 'gadmAdm2':
        ...
        intersect_polys = http.request('GET', urls.TreePlantations,
                                       fields=query)

    else:
        raise AssertionError('Specified intersect_layer ({}) \
                              does not exist.'.format(layer))
